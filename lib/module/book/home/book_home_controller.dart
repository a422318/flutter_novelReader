...
class BookHomeController extends GetxController with WidgetsBindingObserver {
  ...
  getBookList() async {
    ...
    for (var book in books) {
      if (book.type != 1) {
        ...
        var _cur = await _chapterDbProvider.getCurChapterCount(
            book.id, book.curChapter);
    ...
    Get.dialog(
            DialogBuild(
              ...
              confirmFunction: () async {
                ...
                _parseNetworkBook = ParseNetworkBook(
                    url, mainIsolateReceivePort.sendPort,
                    name: bookName);
  ...
  downloadBook(int bookId, int chapterId) async {
    int index =
        _bookDownloads.indexWhere((element) => element.book.id == bookId);
    if (index >= 0) {
    ...
    List<Chapter> chapters =
        await _chapterDbProvider.getChapters(chapterId, bookId);
    if (book == null) {
  ...
  _downloadBook(BookWithChapters bookWithChapters) async {
    for (var chapter in bookWithChapters.chapters) {
      if (bookWithChapters.interruptDownload) {
        ...
        _bookDownloads.removeWhere(
            (element) => element.book.id == bookWithChapters.book.id);
        break;
      }
      var _chapter = await _chapterDbProvider.getChapterById(chapter.id);
      if (_chapter != null &&
          _chapter.content != null &&
          _chapter.content!.isNotEmpty) {
        bookWithChapters.downloadChaptersAdd(chapter);
        continue;
      }
      Chapter? nextChapter = await _chapterDbProvider.getNextChapter(
          chapter.id, bookWithChapters.book.id);
      String content = await HtmlParseUtil.parseContent(
          chapter.name!, chapter.url!, nextChapter?.url);
      if (content.isEmpty) {
        // 下载失败
        continue;
      }
  ...
  Future parseBookText(List<String> lines, String fileName,
  ...
  static _parseBookText(data) async {
    try {
      ...
      for (int i = 0; i < lines.length; i++) {
        if (i % 100 == 0) {
          sendPort
              .send(Message(MessageType.parseTextBook, i * 100 / lines.length));
        }
    ...
    Future.delayed(const Duration(milliseconds: 500), () async {
      ...
      if (path != null) {
        ...
        if (Platform.isIOS) {
          ...
          name =
              Uri.decodeComponent(_path.substring(_path.lastIndexOf("/") + 1));
  ...
  void _iosBrightnessChange(bool toDefault) async {
    ...
    if (toDefault) {
      ...
      if (onBrightness >= 0 &&
          Get.currentRoute != Routes.bookHome &&
          !autoBrightness) {
                ...
                children: [
                  TextSpan(
                      ...
                      recognizer: TapGestureRecognizer()
                        ..onTap = () async {
                          String url =
                              "http://book.private.woshilll.top/book_private.html";
