...
class HtmlParseUtil {
  static final List<String> ignoreContentHtmlTag = [
    "a",
    "option",
    "h1",
    "h2",
  ...
  static Future<List<dynamic>> parseChapter(String url,
      {Function(String? url)? img,
      Function(String name)? name,
      Function(int page)? pageFunc,
      String? originUrl,
      int page = 1}) async {
    ...
    for (var a in aTags) {
      if (a.text.contains(nextPageReg)) {
        ...
        var nextPageData = await parseChapter(nextPageUrl,
            originUrl: originUrl, page: page + 1, pageFunc: pageFunc);
  ...
  static List<Chapter> format(List<Map<String, dynamic>> chapters, String url) {
    ...
    for (var element in chapters) {
      ...
      returnChapters.add(
          Chapter(name: element["name"].trim(), url: url + element["url"]));
    }
    return returnChapters;
  }

  static Future<String> parseContent(
      String chapterName, String url, String? nextChapterUrl,
      {String? originPageId, int maxPage = 0}) async {
    try {
      ...
      for (var a in body.getElementsByTagName("a")) {
        ...
        if (a.text.contains(nextPageReg)) {
          ...
          if (nextChapterUrl != null && nextChapterUrl.isNotEmpty) {
            if (nextPageUrl.contains(nextChapterUrl) ||
                nextPageUrl == nextChapterUrl) {
              break;
            }
          }
          await Future.delayed(const Duration(milliseconds: 1500));
          var nextPageContent = await parseContent(
              chapterName, nextPageUrl, nextChapterUrl,
              originPageId: originPageId, maxPage: maxPage + 1);
          content += nextPageContent;
          break;
        }
      }
      return _removeChapterName(
          chapterName,
          _beautifulFormat(_beautyUnknownTag(
              _beautyNotes(_beautyScript(_beautyBrAndP(_trim(content)))))));
    } catch (err) {
      Log.e(err);
      return "";
    }
  ...
  static Element? findMaxChineseElement(List<Element> elements) {
    ...
    for (var element in elements) {
      var content =
          element.innerHtml.replaceAll(RegExp(r"<a+.*?>([\s\S]*?)</a>"), "");
  ...
  static Future<String?> getString(String url,
      {bool retry = false, int retryTimes = 0}) async {
